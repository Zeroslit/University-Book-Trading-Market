      ...
      <div class="row-between wrap">
        <div>
          ...
          <p class="small muted" style="margin:4px 0 0">
            {{ $t('本校专属图书库与论坛 · 数据严格按 school_id 隔离，你看不到其他学校的帖子与教材') }}
          </p>
        </div>
        <div class="chips">
          <span class="chip">{{ $t('服务费 {0}', [feeText]) }}</span>
          <span class="chip">{{ school?.require_student_verification ? $t('需要学生认证') : $t('免认证') }}</span>
          <span class="chip" :class="{ active: school?.cross_school_enabled }">{{ $t('跨校专区：{0}', [school?.cross_school_enabled ? (school.cross_school_mode === 'mail' ? $t('仅邮寄') : $t('已开启')) : $t('未开启')]) }}</span>
          <span class="chip">{{ $t('信誉分 {0}', [auth.user?.creditScore]) }}</span>
        </div>
      </div>

      <div class="chips mt12">
        <RouterLink class="btn btn-primary btn-sm" to="/books/new">{{ $t('＋ 发布教材') }}</RouterLink>
        <RouterLink class="btn btn-sm" to="/threads/new">{{ $t('＋ 发帖求书 / 转让') }}</RouterLink>
        <RouterLink class="btn btn-sm" to="/orders">{{ $t('我的订单') }}</RouterLink>
        <RouterLink class="btn btn-sm" to="/wallet">{{ $t('钱包（¥{0}）', [yuan(auth.wallet?.balanceCents)]) }}</RouterLink>
        <RouterLink class="btn btn-sm" to="/support">{{ $t('客服中心') }}</RouterLink>
      </div>

      <p v-if="auth.user?.verificationStatus !== 'approved'" class="notice notice-info mt12">
        {{ $t('你的学生认证状态：') }}<b>{{ auth.user?.verificationStatus === 'pending' ? $t('审核中') : $t('未认证') }}</b>{{ $t('。 认证通过前仅可浏览，不能发布与交易。') }}
        <RouterLink to="/verification">{{ $t('去认证 →') }}</RouterLink>
      </p>
    </div>

    <div class="card">
      <div class="row-between">
        <div class="bold">{{ $t('本校最新教材') }}</div>
        <RouterLink class="small" to="/books">{{ $t('查看全部 →') }}</RouterLink>
      </div>
      ...
      <EmptyState v-else :text="$t('本校还没有教材，去发布第一本吧')" icon="📚" />
    </div>

    <div class="card">
      <div class="row-between">
        <div class="bold">{{ $t('本校论坛热帖') }}</div>
        <RouterLink class="small" to="/threads">{{ $t('查看全部 →') }}</RouterLink>
      </div>
      <div v-if="threads.length" class="mt12">
        <div v-for="t in threads" :key="t.id" class="row-between" style="border-bottom:1px solid var(--border);padding:8px 0">
          ...
          <span class="small muted">{{ $t('{0} 回复', [t.reply_count]) }}</span>
        </div>
      </div>
      <EmptyState v-else :text="$t('本校论坛还没有帖子')" icon="💬" />
    </div>

    <div class="card">
      <div class="row-between">
        <div class="bold">{{ $t('我最近的订单') }}</div>
        <RouterLink class="small" to="/orders">{{ $t('全部订单 →') }}</RouterLink>
      </div>
      ...
      <EmptyState v-else :text="$t('还没有订单')" icon="🧾" />
    </div>
  </div>
</template>
