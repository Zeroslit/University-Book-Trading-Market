    ...
    <div class="card">
      <div class="row-between wrap">
        ...
        <div class="tabs" style="margin:0">
          <button class="tab" :class="{ active: role === 'buyer' }" @click="setRole('buyer')">我买到的</button>
          <button class="tab" :class="{ active: role === 'seller' }" @click="setRole('seller')">我卖出的</button>
        </div>
      </div>
      <div class="chips mt12">
        <button class="chip" :class="{ active: !status }" @click="setStatus('')">全部</button>
    ...
    <div v-if="list.length" class="mt12">
      <div v-for="o in list" :key="o.id" class="card">
        ...
        <div class="small muted">
          金额 ¥{{ yuan(o.amount_cents) }} · 服务费 ¥{{ yuan(o.service_fee_cents) }} ·
          {{ SHIP_MODE[o.ship_mode] || o.ship_mode }} · {{ formatTime(o.created_at) }}
        </div>
        <div class="small muted">
          {{ role === 'buyer' ? `卖家：${o.seller_nickname}` : `买家：${o.buyer_nickname}` }}
          <span v-if="o.auto_confirm_at"> · 自动确认：{{ formatTime(o.auto_confirm_at) }}（{{ countdownText(o.auto_confirm_at) }}）</span>
        </div>
      </div>
    </div>
    <EmptyState v-else text="暂无订单" icon="🧾" />
