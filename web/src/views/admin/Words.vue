  ...
  <div>
    <div class="card">
      <div class="row-between wrap">
        <div>
          <div class="bold">违禁词库（四级）</div>
          <div class="small muted">
            L1 提示（打码提醒）· L2 拦截（拦截并提示修改）· L3 违规（拦截 + 扣分 + 记违规）· L4 严重（删除内容 + 转人工工单 + 视情节封禁）
          </div>
        </div>
        <div class="chips">
      ...
      <div class="grid mt12" style="grid-template-columns:repeat(auto-fill,minmax(140px,1fr))">
        <div><label>词条</label><input v-model.trim="form.word" placeholder="展示名，如 手机号" /></div>
        <div><label>匹配方式</label>
          <select v-model="form.matchType"><option value="keyword">关键词</option><option value="regex">正则</option></select>
        </div>
        <div><label>级别</label>
        ...
        <div><label>作用域</label>
          <select v-model="form.scope">
            <option value="school">本校</option>
            <option v-if="auth.isPlatformAdmin" value="global">全局</option>
          </select>
        </div>
        <div><label>分类</label><input v-model.trim="form.category" placeholder="如 引流/盗版" /></div>
        <div><label>扣分（负数为扣分，可空）</label><input v-model.trim="form.scoreDelta" type="number" /></div>
      </div>
      <label v-if="form.matchType === 'regex'">正则表达式内容（如 1[3-9]\\d{9}）</label>
      <input v-if="form.matchType === 'regex'" v-model.trim="form.pattern" />
      <label>备注</label>
      <input v-model.trim="form.remark" />
      <button class="btn btn-primary mt12" @click="create">新增词条</button>
      <p class="small muted mt8">
        归一化检测：去空格与符号、全角转半角、繁简转换、拼音谐音还原；命中会记录 word_hits 便于人工复核。
      </p>
    </div>

    <div class="card">
      <table class="table">
        <thead><tr><th>ID</th><th>词条</th><th>级别</th><th>方式</th><th>分类</th><th>处置</th><th>扣分</th><th>作用域</th><th>状态</th><th>操作</th></tr></thead>
        <tbody>
          <tr v-for="w in words" :key="w.id">
            ...
            <td>{{ w.match_type === 'regex' ? '正则' : '关键词' }}</td>
            <td class="small">{{ w.category || '—' }}</td>
            <td class="small">{{ w.action }}</td>
            <td>{{ w.score_delta ?? 0 }}</td>
            <td class="small">{{ w.scope === 'global' ? '全局' : `本校 #${w.school_id}` }}</td>
            <td>{{ w.status }}</td>
            <td>
              <button class="btn btn-sm" @click="toggle(w)">{{ w.status === 'active' ? '停用' : '启用' }}</button>
              <button class="btn btn-sm" @click="toggleWhite(w)">{{ w.status === 'whitelist' ? '移出白名单' : '白名单' }}</button>
              <button class="btn btn-sm btn-danger" @click="remove(w)">删除</button>
            </td>
          </tr>
        </tbody>
      </table>
      <EmptyState v-if="words.length === 0" text="词库为空" icon="🚫" />
    </div>

    <div class="card">
      <div class="bold">命中记录（证据链）</div>
      <select v-model="filters.hitLevel" style="width:auto;margin-top:8px" @change="loadHits">
        <option value="">全部级别</option>
        <option value="L1">L1</option><option value="L2">L2</option><option value="L3">L3</option><option value="L4">L4</option>
      </select>
      <table class="table mt8">
        <thead><tr><th>时间</th><th>用户</th><th>命中词</th><th>级别</th><th>场景</th><th>目标</th><th>匹配文本</th><th>位置</th><th>处置</th></tr></thead>
        <tbody>
...
async function create() {
  if (!form.word) { toastError('请填写词条名'); return; }
  if (form.matchType === 'regex' && !form.pattern) { toastError('请填写正则表达式'); return; }
  try {
...
async function remove(w) {
  if (!window.confirm(`确认删除词条「${w.word}」？`)) return;
  try {
