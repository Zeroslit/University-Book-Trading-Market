<template>
  <div class="card">
    <h3 style="margin-top:0">发布帖子</h3>
    <p class="small muted">标题与正文都会经过违禁词归一化检测（去空格符号、全角半角、繁简、拼音谐音）。</p>

    <label>帖子类型</label>
    <select v-model="form.type">
      <option value="seek">求书帖（我想买某本书）</option>
      <option value="sell">转让帖（我要转让教材）</option>
    </select>

    <label>标题 *</label>
    <input v-model.trim="form.title" maxlength="200" placeholder="如：求《数据结构（C 语言版）》一本" />

    <label>分类（可选）</label>
    <input v-model.trim="form.category" maxlength="40" placeholder="如 教材 / 考研 / 考证" />
...
async function submit() {
  error.value = '';
  maskedHits.value = [];
  if (form.title.length < 4) { error.value = '标题至少 4 个字'; return; }
  if (form.content.length < 4) { error.value = '正文至少 4 个字'; return; }
