<template>
  <div>
    <div class="card">
      <div class="bold">{{ $t('处罚申诉') }}</div>
      <p class="small muted">
        {{ $t('每一笔处罚都可申诉：处罚后 3 天为申诉期，复审通过将撤销处罚并回滚信誉分。 封禁期间进行中的订单仍可正常完成或退款，资金不会被锁死。') }}
      </p>
      <p class="small muted">{{ $t('申诉次数口径：仅统计「裁定成立」的记录；轻度违规 12 个月滚动过期，连续 6 个月无违规可消除 1 次。') }}</p>
    </div>

    <div class="card">
      <div class="bold">{{ $t('我的处罚记录') }}</div>
      <div v-if="penaltyNotices.length === 0" class="small muted">{{ $t('暂无处罚记录 🎉') }}</div>
      <div v-for="n in penaltyNotices" :key="n.id" class="card" style="margin-top:8px">
        ...
        <label>{{ $t('申诉理由（不少于 5 个字）') }}</label>
        <textarea v-model.trim="appealReasons[n.related_id]" maxlength="1500" />
        <button class="btn btn-sm btn-primary mt8" @click="submitAppeal(n.related_id)">{{ $t('提交申诉') }}</button>
      </div>
    </div>

    <div class="card">
      <div class="bold">{{ $t('我的申诉记录') }}</div>
      <table class="table mt8">
        <thead><tr><th>{{ $t('申诉单号') }}</th><th>{{ $t('处罚 ID') }}</th><th>{{ $t('理由') }}</th><th>{{ $t('状态') }}</th><th>{{ $t('复审说明') }}</th><th>{{ $t('提交时间') }}</th></tr></thead>
        <tbody>
      ...
      <EmptyState v-if="appeals.length === 0" :text="$t('还没有申诉记录')" icon="⚖️" />
    </div>
  </div>
</template>
...
async function submitAppeal(penaltyId) {
  ...
  if (!reason || reason.length < 5) { toastError(t('申诉理由不少于 5 个字')); return; }
  try {
    await reportApi.appeal({ penaltyId: Number(penaltyId), reason });
    toastOk(t('申诉已提交，复审结果将通过站内信通知'));
    appealReasons[penaltyId] = '';
    await load();
  } catch (err) {
    toastError(err?.message || t('申诉提交失败'));
  }
}
